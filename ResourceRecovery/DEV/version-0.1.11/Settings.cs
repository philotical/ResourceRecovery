...
namespace Philotical
{
    ...
    internal class RRSettingsController 
    {
        ...
        private static string PluginName = "ResourceRecovery";
        private static string PartModuleName = "ResourceRecoveryPlanetCore";
        private static string PluginVersion = "0.1.11.0";
        private static string PluginDate = "21.05.2014";
        private static string KSCPlanet = "Kerbin";
        ...
        {   // ConfigDefaults
            RRSettings = RRSettingsController.getRRSettings();
            //Debug.Log("ResourceRecovery getSetting RRSettings=" + RRSettings);
            ConfigNode RRInfo = RRSettings.GetNode("RRInfo");
            //Debug.Log("ResourceRecovery getSetting RRInfo=" + RRInfo);
            string p = RRInfo.GetValue(settingName);
            //Debug.Log("ResourceRecovery getSetting  settingName=" + settingName+" =" + p);
            // ConfigDefaults
            return p;
        }
        internal static string getFilePath(string FilePathName)
        {   // ConfigDefaults
            RRSettings = RRSettingsController.getRRSettings();
            FilePaths = RRSettingsController.getFilePaths();
            string p = FilePaths.GetValue(FilePathName);
            // ConfigDefaults
            return p;
        }
        ...
        {   // ConfigDefaults
            string p = "KSC: 285.442330134503/-0.0972077848128505";
            // ConfigDefaults
            return p;
        }
       ...
       private static void RRSettings_create()
        {
            ...
            RRInfo.AddValue("PluginName", PluginName);
            RRInfo.AddValue("PartModuleName", PartModuleName);
            RRInfo.AddValue("PluginVersion", PluginVersion);
            RRInfo.AddValue("PluginDate", PluginDate);
            RRInfo.AddValue("ConfigFileWrittenAt", DateTime.Now.ToString());
            ConfigNode FPaths = new ConfigNode("FilePaths");
            FPaths.AddValue("KSCPlanet", KSCPlanet);
