...
{
    class ManifestController : MonoBehaviour
    {
        ...
        internal void ManifestMain(int id, ScenarioNodeDatabase Sc)
        {
            Scenario = Sc;
        ...
        */
        ...
        internal bool DEBUG_FillResource(string res_nam,string current)
        {
            ...
            if (Double.TryParse(current, out string_number2))
            {
                ...
                if (Scenario.modify_ResourceNode_by_ResourceName(res_nam, "storedamount", string_number2.ToString()))
                {
                    //RootNode = Scenario.get_RootNode();
                    if (Scenario.save())
                    {
                        Debug.Log("ResourceRecovery DEBUG_FillResource Scenario.save(RootNode) worked");
                        return true;
                    }
                    else
                    {
                        Debug.Log("ResourceRecovery EXCEPTION: DEBUG_FillResource SavePluginSaveFile failed");
                        return false;
                    }
                }
                return false;
        ...
        internal bool changeGroup(string res_nam, string current)
        {
            ...
            */
            if (Scenario.modify_ResourceNode_by_ResourceName(res_nam, "group", current))
            {
                //Scenario.update_StorageTotals();
                //RootNode = Scenario.get_RootNode();
                if (Scenario.save())
                {
                    //Debug.Log("ResourceRecovery DumpResource Scenario.save(RootNode) worked");
                    return true;
                }
                else
                {
                    //Debug.Log("ResourceRecovery EXCEPTION: DumpResource SavePluginSaveFile failed");
                    return false;
                }
            }
            return false;
        ...
        {
            if (Scenario.modify_ResourceNode_by_ResourceName(res_nam, "storedamount", "0.00"))
            {
                //Scenario.update_StorageTotals();
                //RootNode = Scenario.get_RootNode();
                if (Scenario.save())
        ...
        internal bool change_hidden_status(string res_nam, string current)
        {
            ...
            if (Scenario.modify_ResourceNode_by_ResourceName(res_nam, "hide", current))
            {
                //Scenario.update_StorageTotals();
                //RootNode = Scenario.get_RootNode();
                if (Scenario.save())
                {
                    //Debug.Log("ResourceRecovery change_hidden_status Scenario.save(RootNode) worked");
                    return true;
                }
                else
                {
                    //Debug.Log("ResourceRecovery EXCEPTION: change_hidden_status SavePluginSaveFile failed");
                    return false;
                }
            }
            return false;
        ...
        internal bool change_order(string res_nam, int old_id, int new_id)
        {
            ...
            */
            ...
            if (ResourceNode1.SetValue("display_order", new_id_string) && ResourceNode2.SetValue("display_order", old_id_string))
            {
                ...
                if (Scenario.save())
                {
                    //Debug.Log("ResourceRecovery change_order Scenario.save(RootNode) worked");
                    return true;
                }
                else
                {
                    //Debug.Log("ResourceRecovery EXCEPTION: change_order SavePluginSaveFile failed");
                    return false;
                }
            }
            return false;
